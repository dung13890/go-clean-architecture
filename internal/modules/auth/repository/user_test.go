...
var errUserRp = errors.New("Have an error")
...
func setUpUser(t *testing.T) (*sql.DB, *gorm.DB, sqlmock.Sqlmock) {
	t.Helper()
...
func TestFetchUserToParallel(t *testing.T) {
	t.Parallel()
	db, dbConnectMock, mock := setUpUser(t)
	defer db.Close()

	repoMock := &repository.UserRepository{
		DB: dbConnectMock,
	}
	...
	tcs := []userTestcase{
		...
		},
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				...
				mock.ExpectQuery(query).WithArgs().WillReturnError(errUserRp)
			},
			res: ([]domain.User)(nil),
			err: errUserRp,
		},
	...
	tcs := []userTestcase{
		...
		},
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				...
				mock.ExpectQuery(query).WithArgs(2).WillReturnError(errUserRp)
			},
			args: &domain.User{
				ID: 2,
			},
	...
	tcs := []userTestcase{
		...
		},
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				...
				mock.ExpectQuery(query).WithArgs("email2").WillReturnError(errUserRp)
			},
			args: domain.User{
				Email: "email2",
			},
		...
		},
		{
			...
			mock: func(mock sqlmock.Sqlmock) {
				...
				mock.ExpectQuery(query).WithArgs("email3").WillReturnError(gorm.ErrRecordNotFound)
			},
			args: domain.User{
				Email: "email3",
			},
